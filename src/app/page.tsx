...
const MainContent: React.FC = () => {
  ...
  const memoizedLanyard = useMemo(() => (
    <Lanyard
      ...
      frontImage="/assets/bilder/eren-photo.png"
      backImage="/assets/bilder/l2.jpg"
    />
  ), []);
  ...
  useGSAP(() => {
    ...
    // The photo card is the 3D lanyard now — it drops in via its own physics
    // simulation, so there is no entrance tween for it here.
    ...
    <div suppressHydrationWarning className="min-h-screen bg-grid-mesh relative text-[var(--text-body)] bg-[var(--background)] transition-colors duration-300 flex flex-col overflow-x-hidden w-full">
      ...
      <main ref={mainRef} suppressHydrationWarning className="flex-1 max-w-6xl w-full mx-auto px-6 pt-24 md:pt-32 pb-12 md:pb-20 space-y-20 md:space-y-28 overflow-x-hidden">
        ...
        >
          ...
          {/* Right Column: Premium Photo Card on a draggable lanyard.
              Deliberately NOT GSAP-animated: react-three-fiber sizes the canvas
              from the container's getBoundingClientRect(), so a rotate/scale
              tween on any ancestor makes it latch onto the rotated bounding box
              (600x800 at rotationZ 15 / scale 0.8 measures 629x742) and never
              re-measure. The card has its own physics entrance anyway. */}
          <div
            className="flex-shrink-0 z-[45] relative w-[300px] h-[380px] lg:-translate-x-4 lg:translate-y-4"
          >
            {/* Absolutely positioned so the oversized canvas doesn't push the text column */}
            <div className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-[400px] h-[650px] md:w-[600px] md:h-[800px] -mt-24 md:-mt-32 pointer-events-auto z-[45]">
              {memoizedLanyard}
            </div>
          </div>
        </section>
