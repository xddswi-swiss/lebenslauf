...
async function saveToGitHub(
  ...
  try {
    ...
    console.log(
      `File ${filePath} does not exist on GitHub yet or error fetching SHA. Creating as new file.`,
    );
  }
  ...
  try {
    ...
    if (!company || !city || !period || !de?.role) {
      return NextResponse.json(
        {
          error:
            "Fehlende Felder / Eksik alanlar / Missing required fields (Company, City, Period and German Role are required)",
        },
        { status: 400 },
      );
    }
    ...
    if (logoFile && logoFile.base64 && logoFile.name) {
      ...
      try {
        ...
        if (githubToken) {
          await saveToGitHub(
            `public/assets/bilder/${logoFileName}`,
            base64Data,
    ...
    if (pdfFile && pdfFile.base64 && pdfFile.name) {
      ...
      try {
        ...
        if (githubToken) {
          await saveToGitHub(
            `public/assets/pdfs/${pdfFileName}`,
            base64Data,
            `Upload PDF report for ${company}`,
          );
        }
      } catch (err: any) {
    ...
    const { error: dbError } = await supabaseAdmin.from("experiences").insert({
      ...
      tr: {
        role: tr?.role?.trim() || de.role.trim(),
        tasks: tr?.tasks || de.tasks || [],
      },
      en: {
        role: en?.role?.trim() || de.role.trim(),
        tasks: en?.tasks || de.tasks || [],
      },
      logo_path: logoPath,
      pdf_path: pdfPath,
    });
...
export async function DELETE(request: Request) {
  try {
    ...
    if (!company || !period) {
      return NextResponse.json(
        { error: "Missing company or period" },
        { status: 400 },
      );
    }
