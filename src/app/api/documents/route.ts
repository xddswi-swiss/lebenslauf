  ...
  try {
    ...
    if (!deTerm || !date || !pdfFile || !pdfFile.base64 || !pdfFile.name) {
      return NextResponse.json(
        {
          error:
            "Fehlende Felder / Eksik alanlar / Missing required fields (Title, Year and PDF file are required)",
        },
        { status: 400 },
      );
    }
...
export async function DELETE(request: Request) {
  try {
    ...
    const { error: dbError } = await supabaseAdmin
      ...
      .delete()
      .or(
        `de_term.eq."${term.trim()}",tr_term.eq."${term.trim()}",en_term.eq."${term.trim()}"`,
      );
