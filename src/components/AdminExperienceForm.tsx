...
export const AdminExperienceForm: React.FC<{ forceOpen?: boolean }> = ({
  forceOpen = false,
}) => {
    ...
    try {
      ...
      if (res.ok) {
        ...
        window.dispatchEvent(
          new CustomEvent("experiences-updated", {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    const fallbackTasks =
      tasksText.de.trim() || tasksText.tr.trim() || tasksText.en.trim();
    ...
    try {
      const [
        ...
        roles.de.trim()
          ? Promise.resolve(roles.de.trim())
          : translateText(fallbackRole, roleSrcLang, "de"),
        roles.tr.trim()
          ? Promise.resolve(roles.tr.trim())
          : translateText(fallbackRole, roleSrcLang, "tr"),
        roles.en.trim()
          ? Promise.resolve(roles.en.trim())
          : translateText(fallbackRole, roleSrcLang, "en"),
        tasksText.de.trim()
          ? Promise.resolve(tasksText.de.trim())
          : translateText(fallbackTasks, tasksSrcLang, "de"),
        tasksText.tr.trim()
          ? Promise.resolve(tasksText.tr.trim())
          : translateText(fallbackTasks, tasksSrcLang, "tr"),
        tasksText.en.trim()
          ? Promise.resolve(tasksText.en.trim())
          : translateText(fallbackTasks, tasksSrcLang, "en"),
      ]);
      ...
      if (result.readOnly) {
        ...
        resetForm();
        setSuccessMsg(
          successAlertMsg[language as "de" | "tr" | "en"] || successAlertMsg.tr,
        );
  ...
  const labels = {
    tr: {
      ...
      fallbackDesc:
        'Proje şu an salt okunur bir sunucuda (Vercel gibi) barındırılmaktadır. Bu sebeple veriler doğrudan kaydedilemedi. Lütfen aşağıdaki JSON kodunu kopyalayın ve projenizin içindeki "src/data/experiences.json" dosyasının içeriği ile tamamen değiştirin.',
    ...
    de: {
      ...
      passDesc:
        "Geben Sie das Passwort ein, um eine neue Schnupperlehre hinzuzufügen.",
      ...
      fallbackDesc:
        'Dieses Projekt läuft auf einem schreibgeschützten Server (z.B. Vercel). Die Daten konnten nicht direkt gespeichert werden. Bitte kopieren Sie den folgenden JSON-Code und ersetzen Sie damit den Inhalt der Datei "src/data/experiences.json" in Ihrem Projekt.',
    ...
    en: {
      ...
      fallbackDesc:
        'This project is running on a read-only host (e.g. Vercel). The files could not be updated directly. Please copy the JSON below and replace the content of your "src/data/experiences.json" file with it.',
      btnCopy: "Copy Code",
      btnCopied: "Copied!",
    },
  };
              ...
              <form onSubmit={handleSubmit} className="space-y-6">
                ...
                <div className="space-y-1.5 max-w-xs pt-1">
                  ...
                  <select
                    value={type}
                    onChange={(e) =>
                      setType(e.target.value as "work" | "education")
                    }
                  ...
                  >
                    <option value="work">
                      {language === "tr"
                        ? "Staj / Deneyim"
                        : language === "de"
                          ? "Schnupperlehre (Erfahrung)"
                          : "Trial Apprenticeship (Experience)"}
                    </option>
                ...
                {(() => {
                  ...
                  const companyPlaceholder =
                    type === "work"
                      ? "e.g. EWZ, Schibli AG"
                      : language === "tr"
                        ? "Örn: Sekundarschule Zürich Rebhügel"
                        : language === "de"
                          ? "Z.B. Sekundarschule Zürich Rebhügel"
                          : "e.g. Sekundarschule Zurich Rebhugel";

                  const cityPlaceholder =
                    type === "work" ? "e.g. Zürich" : "e.g. Zürich";
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                      <div className="space-y-1.5">
                        ...
                        <input
                          ...
                          placeholder={
                            type === "work"
                              ? "z.B. Schnupperlehre Elektroinstallateur"
                              : "z.B. Sekundarschule (Sek A)"
                          }
                          className="w-full px-4 py-3 rounded-xl bg-[var(--background)] border border-[var(--glass-border)] focus:border-primary focus:outline-none text-[var(--text-main)] text-sm transition-all"
                        />
                      </div>
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                      <div className="space-y-1.5">
                        ...
                        <input
                          ...
                          placeholder={
                            type === "work"
                              ? "Örn: Elektrik Tesisatçısı Stajı"
                              : "Örn: Sekundarschule (Sek A)"
                          }
                          className="w-full px-4 py-3 rounded-xl bg-[var(--background)] border border-[var(--glass-border)] focus:border-primary focus:outline-none text-[var(--text-main)] text-sm transition-all"
                        />
                      </div>
                    ...
                    {existingExperiences.map((item, idx) => (
                      ...
                      >
                        <div className="min-w-0 flex-1">
                          ...
                          <p className="text-xs text-[var(--text-body)] mt-0.5 font-medium">
                            {item.role}
                          </p>
                        ...
                        <button
                          type="button"
                          onClick={() =>
                            handleDeleteExperience(item.company, item.period)
                          }
